-- Comprehensive function to create interview summary from analysis data
...
BEGIN
  ...
  improvement_counts AS (
    ...
    ORDER BY count DESC, improvement
    LIMIT 5
  )
  SELECT 
    COALESCE(jsonb_agg(strength ORDER BY count DESC, strength), '[]'::jsonb),
    COALESCE(jsonb_agg(improvement ORDER BY count DESC, improvement), '[]'::jsonb)
...
-- Function to get comprehensive session analytics
...
COMMENT ON FUNCTION create_interview_summary(UUID) IS 'Creates or updates interview summary by aggregating data from interview_analysis table';
COMMENT ON FUNCTION get_session_analytics(UUID) IS 'Returns comprehensive analytics for a specific interview session';
